...
def get_quantized_models(model_name):
    ...
    # model_nf4.gradient_checkpointing_enable()
...
def reward_func(**kwargs):
    return [1.0 if gt == c else 0.0 for c, gt in zip(kwargs['completions'], kwargs['answer'])]
...
def main():
    ...
    evalDatasetFromList = Dataset.from_list(list(map(lambda x: formatting_prompts(x), random.sample(prompt_data_val, 100))))
