...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='./models/DPO/Qwen8B_DPO_d1', help='output directory')
    parser.add_argument('--input_dataset', type=str, default='./sft_data/sft_train_d1_selected_all_paths.pkl', help='dataset file')
    parser.add_argument('--val_dataset', type=str, default='./sft_data/sft_train_val_d1_selected_all_paths.pkl', help='val dataset file')
    parser.add_argument('--learning_rate', type=float, default=1e-6, help='learning rate')
    args = parser.parse_args()
    return args

def formatting_prompts(textData):
    # print("text", textData.keys())
    return {"prompt": f"{textData['prompt']}", "chosen": f"{textData['answer']}", "rejected": f"{textData['wrong_options']}"}
