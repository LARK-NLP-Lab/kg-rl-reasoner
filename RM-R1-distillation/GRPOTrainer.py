...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default="./models/Qwen7B_orca_sft/", help='model name')
    parser.add_argument('--output_dir', type=str, default='./models/Qwen7B_orca_grpo', help='output directory')
...
def formatting_prompts(textData):
    return {"prompt": f"{textData['prompt']}", "completion": f"{textData['answer']}", "label_path": f"{textData['label_path']}"}
...
def get_quantized_models(model_name):
    ...
    model_nf4 = AutoModelForCausalLM.from_pretrained(model_name, device_map="auto", offload_folder="offload/") 
...
def peft_model(model, model_name, is_pretrained):
    if is_pretrained:
        model = PeftModel.from_pretrained(model=model, model_id=model_name, is_trainable=True, offload_folder="offload/")
    else:
...
def reward_func(**kwargs):
    ...
    # print("completions", kwargs['completion_ids'])
    # print("ground_truth", kwargs['label_path'])
    return [1.0 if f"<CorrectPaths>{label_path}</CorrectPaths>" in c else 0.0 for c, label_path in zip(kwargs['completions'], kwargs['label_path'])]
